...
ACTUATOR_IMAGE="wharttest-250-actuator:update-01339484-arm64-r3"
...
if ! docker image inspect "$ACTUATOR_IMAGE" >/dev/null 2>&1; then
  ...
  echo "请先执行：" >&2
  echo "cat $PACKAGE_DIR/images/actuator-update-01339484-arm64-r3.tar.gz.part* | gzip -dc | docker load" >&2
  exit 1
fi
...
if [ ! -s "$SECRET_FILE" ]; then
  read -r -s -p "请输入平台账号 ${ACTUATOR_API_USERNAME:-admin} 的登录密码：" password
  echo
