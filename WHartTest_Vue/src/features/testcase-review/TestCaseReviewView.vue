        ...
        <div v-else class="review-list">
          <article v-for="item in reviews" :key="item.id" class="review-item">
            ...
            <div class="review-main">
              ...
              <div v-if="item.status === 'failed' && resumeHint(item)" class="coverage-warning">{{ resumeHint(item) }}</div>
              <div v-if="item.status === 'completed' && item.summary?.uncovered_chunks" class="coverage-warning">
                有 {{ item.summary.uncovered_chunks }} 批未成功审查，约 {{ item.summary.uncovered_rows || 0 }} 行未被覆盖，详见报告「未覆盖用例行」
              </div>
            </div>
            <div class="actions">
...
function resumeHint(item: TestCaseReview) {
  const chunks = item.summary?.progress_chunks;
  if (!chunks?.total || !chunks.completed) return '';
  return `已完成 ${chunks.completed}/${chunks.total} 批，点击「重试」将从断点继续，无需重新审查已完成的批次。`;
}
