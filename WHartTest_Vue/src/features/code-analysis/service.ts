...
export async function purgeRepositoryCopy(id: number) { const r = await request<any>({ url: `${base}/repositories/${id}/purge-local-copy/`, method: 'POST' }); if (!r.success) throw new Error(r.error); return r.data; }
...
export async function retryTaskOcr(id: string) { const r = await request<AnalysisTask>({ url: `${base}/tasks/${id}/retry-ocr/`, method: 'POST' }); if (!r.success) throw new Error(r.error); return r.data!; }
