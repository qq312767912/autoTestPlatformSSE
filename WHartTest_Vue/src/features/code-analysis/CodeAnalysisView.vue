      ...
      <a-tabs v-if="selectedTask" default-active-key="change" class="report-tabs">
        <a-tab-pane key="change" title="代码审查报告">
          ...
          <div v-for="item in filteredFindings" :key="item.key" class="finding">
            ...
            <div class="code-snippet"><div class="snippet-caption"><span>风险证据</span><small>{{ item.line_start ? `旧版本第 ${item.line_start} 行` : '未定位源码行' }}</small></div><div v-for="(line, index) in evidenceLines(item)" :key="index" class="snippet-line"><span>{{ item.line_start ? item.line_start + index : '—' }}</span><code>{{ line || ' ' }}</code></div></div><div class="impact">影响：{{ item.impact }}</div><div v-if="item.recommendation" class="impact"><b>建议：</b>{{ item.recommendation }}</div><a-link class="diff-link" @click="openDiff(item)"><icon-file /> 查看完整 Diff</a-link>
          </div>
...
const executionEventLabel = (event:string) => ({created:'已创建',queued:'已入队',started:'开始执行',completed:'已完成',partial:'部分完成',failed:'执行失败',cancelled:'已取消'} as any)[event] || event;
