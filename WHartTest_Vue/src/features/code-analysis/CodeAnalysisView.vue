    ...
    <a-drawer :visible="!!selectedTask" :width="780" unmount-on-close @cancel="selectedTask = null">
      ...
      <a-tabs v-if="selectedTask" default-active-key="change" class="report-tabs">
        <a-tab-pane key="change" title="代码审查报告">
          <div class="report-toolbar"><div><h3>风险与影响</h3><p>确定性规则与 AI 风险提示的融合结果</p></div><a-dropdown trigger="click" @select="download(selectedTask, 'change', $event)"><a-button type="outline"><template #icon><icon-download /></template>下载报告<icon-down class="download-arrow" /></a-button><template #content><a-doption value="md">Markdown 文档（.md）</a-doption><a-doption value="html">前端样式网页（.html）</a-doption></template></a-dropdown></div>
      ...
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="分析模式" class="analysis-mode-form-item">
          <a-radio-group v-model="form.mode" class="analysis-mode-selector">
            <a-radio
              v-for="option in analysisModeOptions"
              :key="option.value"
              :value="option.value"
              class="analysis-mode-card"
          ...
          <div class="mode-description"><span>当前模式</span>{{ modeDescription }}</div>
        </a-form-item>
      </a-form>
    </a-modal>
...
const iterationTestRequirements = computed(() => visibleTestRequirements.value
  .filter(item => !riskTestRequirements.value.includes(item))
  .slice()
  .sort((a,b) => (severityOrder[a.priority] ?? 9) - (severityOrder[b.priority] ?? 9)));
...
async function download(task:AnalysisTask,type:'change'|'test',format:string){
  ...
  }catch(error:unknown){Message.error(error instanceof Error ? error.message : '报告下载失败')}
}
...
.impact-module-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:8px;margin-top:14px}.impact-module-grid>div{min-width:0;min-height:68px;padding:11px 12px;border:1px solid rgb(255 255 255 / 17%);border-radius:8px;background:rgb(1 18 29 / 16%)}.impact-module-grid b,.impact-module-grid span,.impact-module-grid small{display:block;max-width:100%;overflow-wrap:anywhere;word-break:break-word}.impact-module-grid b{font-size:13px;line-height:1.5}.impact-module-grid span{margin-top:3px;color:#9fe1dd;font-size:11px}.impact-module-grid small{margin-top:5px;color:#c2d7d9;font-size:11px;line-height:1.55}.diff-link{display:inline-flex;align-items:center;gap:5px;margin-top:11px;font-size:12px}.diff-view{overflow:auto;max-height:66vh;border:1px solid #dce4ea;border-radius:9px;background:#fbfcfd}.diff-line{display:grid;grid-template-columns:54px minmax(0,1fr);min-height:25px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;line-height:1.58}.diff-line>span{padding:3px 10px 3px 4px;border-right:1px solid #e3e8ed;color:#98a5b2;background:#f4f6f8;text-align:right;user-select:none}.diff-line code{padding:3px 12px;white-space:pre-wrap;overflow-wrap:anywhere;background:transparent;color:#344054}.diff-line.added{background:#ecfdf3}.diff-line.added code{color:#12633f}.diff-line.removed{background:#fff1f1}.diff-line.removed code{color:#a12834}.diff-line.hunk{background:#eef6ff}.diff-line.hunk code{color:#2865a8}
...
.analysis-mode-selector{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;width:100%}
.analysis-mode-form-item :deep(.arco-form-item-content-flex){flex-direction:column;align-items:stretch}
.analysis-mode-selector :deep(.arco-radio){margin-right:0}
.analysis-mode-card{position:relative;display:flex;align-items:flex-start;min-width:0;min-height:88px;margin:0;padding:13px 12px;border:1px solid #d8e3ea;border-radius:11px;background:#fff;cursor:pointer;transition:border-color .18s ease,background-color .18s ease,box-shadow .18s ease,transform .18s ease}
.analysis-mode-card::before{position:absolute;inset:9px auto 9px 0;width:3px;border-radius:0 3px 3px 0;background:transparent;content:"";transition:background-color .18s ease}
.analysis-mode-card:hover{border-color:#8cc6d2;background:#f9fcfd;transform:translateY(-1px)}
.analysis-mode-card.is-active{border-color:#1687a7;background:#eaf7fa;box-shadow:0 7px 16px rgb(22 135 167 / 12%);transform:translateY(-2px)}
.analysis-mode-card.is-active::before{background:#1687a7}
.analysis-mode-card :deep(.arco-radio-label){flex:1;min-width:0;padding-left:9px}
.analysis-mode-content{min-width:0}
.analysis-mode-heading{display:flex;align-items:center;justify-content:space-between;gap:7px}
.analysis-mode-heading strong{color:#1f2d3d;font-size:14px;line-height:1.35}
.analysis-mode-heading span{color:#1687a7;font-size:10px;font-weight:600;letter-spacing:.06em;white-space:nowrap}
.analysis-mode-content p{margin:8px 0 0;color:#6b7a8c;font-size:11px;line-height:1.55}
.mode-description{display:flex;align-items:flex-start;gap:8px;width:100%;margin-top:10px;padding:9px 11px;border-radius:8px;background:#f3f7f9;color:#6b7a8c;font-size:12px;line-height:1.55}
.mode-description span{flex:none;color:#1687a7;font-weight:600}
@media(max-width:720px){.analysis-mode-selector{grid-template-columns:1fr}.analysis-mode-card{min-height:76px}}
</style>
