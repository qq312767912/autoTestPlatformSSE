...
4. 重复开启浏览器：在功能测试执行过程中未顺手采集元素，导致执行结束后又重新打开浏览器采集元素或重复执行。

### 探索执行与用例生成时的单次流原则（严禁二次打开浏览器）

在功能测试用例执行并生成 UI 自动化资产的场景下：
1. **同步记录元素定位器**：在浏览器首次操作每个步骤（输入、点击等）时，直接提取并记录该元素的最佳定位器，严禁在执行完毕后再重新开启浏览器去二次抓取。
2. **直接组装入库与绑定**：功能用例执行完毕且断言通过后，直接使用首次操作中已记录的定位器与步骤调用 `create_element` / `create_page_step` / `create_testcase` 组装用例并完成绑定，无需且严禁再次启动浏览器重复执行！
