...
# 内网推理服务吞吐有限；高并发会让多个 Agent 请求同时占用模型，最终全部
# 卡在网关超时。首轮和续审统一单并发，以完成率优先。
OCR_CONCURRENCY = 1
OCR_RESUME_CONCURRENCY = 1
...
def _invalid_ocr_result_reason(payload, result, timed_out, timeout_minutes, output_path):
    ...
    stderr = str(getattr(result, "stderr", "") or "").strip()[-1000:]
    return_code = getattr(result, "returncode", None)
    if timed_out:
        suffix = f"；最后错误：{stderr}" if stderr else ""
        return (
            f"OpenCodeReview 执行超过 {timeout_minutes} 分钟，且未生成有效 JSON 结果"
            f"（退出码 {return_code}，结果文件：{output_path.name}）{suffix}"
        )
    if return_code not in (None, 0):
        return f"OpenCodeReview 异常退出（退出码 {return_code}）：{stderr or '未返回错误文本'}"
    if not output_path.exists():
        return f"OpenCodeReview 未生成结果文件（状态 {status or 'empty'}，退出码 {return_code}）{('；' + stderr) if stderr else ''}"
    return f"OpenCodeReview 返回无效结果（状态 {status or 'empty'}，退出码 {return_code}）{('；' + stderr) if stderr else ''}"
...
def _run_open_code_review(task):
    ...
    try:
        with _ocr_repository(task) as (root, base_ref, head_ref):
            ...
            effective_timeout = primary_timeout
            ...
            if _ocr_needs_resume(payload):
                ...
                try:
                    ...
                    if resumed_payload:
                        ...
                        effective_timeout = resume_timeout
        ...
        if not payload or status not in accepted_statuses:
            raise ValueError(
                ...
                or _invalid_ocr_result_reason(payload, result, timed_out, effective_timeout, output_path)
            )
        manifest = payload.get("manifest") or {}
        coverage = manifest.get("coverage") or {}
